...
namespace Tests
{
    public class CsvPlayerParserTests
    {
        ...
        """.Split("\n");
            ...
            var players = CvsPlayersParser.ParseLines(csvLines);
            Assert.AreEqual(4, players.Count);

            var playerStats = new PlayerDto(players[1]);
        ...
        public void ParseExistentValidCsvFileReturnsPlayers()
        {
            ...
            var filePath = Path.Combine(currentDirectory, @"..\..\..\..\Levi9_competition\Data\Players.csv");

            var players = CvsPlayersParser.Parse(filePath);
        ...
        {
            var currentDirectory = Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location);
            var filePath = Path.Combine(currentDirectory, @"..\..\..\..\Levi9_competition\Data\asdasd.csv");
